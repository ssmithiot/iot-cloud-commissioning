"""IOT Edge Development Updater -- a side-by-side copy of the working updater.

This file is a copy of ``tools/legacy_edge_upgrade_webapp.py``.  The proven
implementation is what makes it trustworthy: the same Cradlepoint connection and
host-key handling, the same nested-SSH path to the gateway behind it, the same
identity validation, component selection, checkpoint creation, update phases,
service restart, verification and rollback reporting.  None of that is
reimplemented here.

Only what separation and the Edge 0.2.0 targets require is changed, and every
change is marked ``DEV-UPDATER:`` so the delta against the original reads in one
pass:

  1. import path, because this module sits one directory deeper
  2. its own port, config directory, log directory and PID file
  3. its own ``.env`` location, with a clear report when it is missing
  4. its own copy of the release manifest
  5. the product title
  6. cloud job claiming off by default -- this is a manual tool

Jim's updater is never imported, read at runtime, or written to.
"""
...
# DEV-UPDATER (1): parents[2], not parents[1] -- this module lives in
# tools/dev_updater/ rather than tools/. The imports themselves are unchanged:
# these are shared, side-effect-free helpers, not Jim's updater.
...
from tools.dev_updater.commit_resolution import EDGE_AGENT_REPOSITORY, CommitResolutionError, resolve_commit
...
# DEV-UPDATER (2): its own port, never 8766.
...
# DEV-UPDATER (4): its own copy of the manifest. Jim's file is never read here.
DEFAULT_RELEASE_MANIFEST = str(Path(__file__).resolve().parent / "releases" / "manifests" / "edge-0.2.0-dev.json")
...
# DEV-UPDATER (4): the operator sees seven characters; checkout, validation,
# deployment and logging all keep using the full 40 above.
DEFAULT_EDGE_UI_COMMIT_SHORT = DEFAULT_EDGE_UI_COMMIT[:7]
DEFAULT_EDGE_UPDATE_REF_SHORT = DEFAULT_EDGE_UPDATE_REF[:7]
DEFAULT_EDGE_UI_BRANCH = "release/edge-ui-0.2.0"
DEFAULT_EDGE_AGENT_BRANCH = "release/edge-agent-0.2.0"
...
class UpgradeRequest:
    ...
    effective_agent_commit: str = DEFAULT_EDGE_UPDATE_REF
...
def embedded_ui_artifact_summary(release_manifest_path: str) -> dict[str, str]:
    ...
    repository_root = Path(__file__).resolve().parents[2]  # DEV-UPDATER (1)
    summary = {
...
# DEV-UPDATER (3): the .env lives in this product's own configuration
# directory, so Steve can copy his working file across without either program
# reading the other's. Same variable names as the original, so the copy needs no
# rewriting. Required names are declared here rather than inferred, so a missing
# one can be named in the report instead of failing later as a blank password.
ENV_FILENAME = ".env"
REQUIRED_ENV_VARS = ("CRADLEPOINT_PASSWORD", "GATEWAY_PASSWORD")
OPTIONAL_ENV_VARS = ("IOT_ADMIN_API_TOKEN", "EDGE_UI_PASSWORD")


def env_path() -> Path:
    return identity.data_dir() / ENV_FILENAME


@dataclass(frozen=True)
class EnvStatus:
    """What was found, never what it contained."""

    path: Path
    exists: bool
    missing_required: tuple[str, ...]
    missing_optional: tuple[str, ...]
    loaded_from_environment: tuple[str, ...]

    @property
    def ok(self) -> bool:
        return self.exists and not self.missing_required

    def message(self) -> str:
        if not self.exists:
            return (
                f"No configuration file at {self.path}.\n"
                f"  Copy your existing updater .env to that path, then start this application again.\n"
                f"  Required variables: {', '.join(REQUIRED_ENV_VARS)}.\n"
                f"  Optional variables: {', '.join(OPTIONAL_ENV_VARS)}.\n"
                f"  See {ENV_FILENAME}.example beside the application for the full list."
            )
        if self.missing_required:
            return (
                f"{self.path} is missing required variable(s): {', '.join(self.missing_required)}.\n"
                f"  Add the name(s) above to that file. Values are never displayed or logged."
            )
        return f"Configuration loaded from {self.path}."


def env_status() -> EnvStatus:
    """Inspect the configuration without returning or printing any value."""
    path = env_path()
    known = REQUIRED_ENV_VARS + OPTIONAL_ENV_VARS
    from_env = tuple(name for name in known if os.environ.get(name, "").strip())
    if not path.exists():
        return EnvStatus(path, False, tuple(n for n in REQUIRED_ENV_VARS if n not in from_env), tuple(n for n in OPTIONAL_ENV_VARS if n not in from_env), from_env)
    present = set(from_env)
    ...
    return EnvStatus(
        path,
        True,
        tuple(name for name in REQUIRED_ENV_VARS if name not in present),
        tuple(name for name in OPTIONAL_ENV_VARS if name not in present),
        from_env,
    )
...
def release_package_status(manifest_path: str) -> str:
    try:
        ...
        artifact = verify_manifest_artifact(manifest, Path(__file__).resolve().parents[2])  # DEV-UPDATER (1)
        validate_embedded_ui_artifact_contents(artifact)
    except Exception as exc:
...
def form_page(message: str = "") -> bytes:
    ...
    # DEV-UPDATER (5): the title, and only the title. The form, the phase list,
    # the component checkboxes, the styling and the workflow below are the
    # working updater's, unchanged.
    env_state = env_status()
    env_banner = "" if env_state.ok else (
        f'<p class="error">Configuration incomplete — {escape(env_state.message().splitlines()[0])}</p>'
    )
    return page(
        identity.PRODUCT_NAME,
        f"""
<h1>{escape(identity.PRODUCT_NAME)}</h1>
<p>Version {escape(identity.APP_VERSION)}</p>
...
<p class="small">Edge UI <code>{escape(DEFAULT_EDGE_UI_COMMIT_SHORT)}</code> ({escape(DEFAULT_EDGE_UI_BRANCH)}) &middot;
Edge Agent <code>{escape(DEFAULT_EDGE_UPDATE_REF_SHORT)}</code> ({escape(DEFAULT_EDGE_AGENT_BRANCH)}) &middot;
port {DEFAULT_PORT}</p>
{env_banner}
{warning}
...
<form id="upgrade-form" method="post" action="/api/start">
  ...
  <label>Edge Agent Git commit
    <input name="edge_agent_commit" placeholder="Optional: full SHA or unambiguous 7+ character prefix; manifest default when blank">
    <span class="hint">A resolved explicit commit is authoritative for this run; otherwise the selected manifest remains authoritative.</span>
  </label>
  <label>Edge Release
  ...
  <div class="wide phase-select">
    ...
    <!-- DEV-UPDATER (7): the same checkboxes and the same selected_phases
         mechanism; these two only tick the sets the original already defines as
         UI_ONLY_PHASES and TARGETED_AGENT_ONLY_PHASES, so "Edge UI only" and
         "Edge Agent only" are one click instead of twelve. "Select all" remains
         the both-components case it always was. -->
    <button type="button" id="select-ui-phases">Edge UI only</button>
    <button type="button" id="select-agent-phases">Edge Agent only</button>
    <div class="phase-groups">
...
// DEV-UPDATER (7): tick exactly the phase numbers the server already treats as
// each component. No new request field and no new server behaviour.
const UI_ONLY_PHASES = {list(UI_ONLY_PHASES)};
const AGENT_ONLY_PHASES = {list(TARGETED_AGENT_ONLY_PHASES)};
const selectOnly = (wanted) => phaseChecks().forEach((input) => input.checked = wanted.includes(Number(input.value)));
document.getElementById("select-ui-phases").addEventListener("click", () => selectOnly(UI_ONLY_PHASES));
document.getElementById("select-agent-phases").addEventListener("click", () => selectOnly(AGENT_ONLY_PHASES));
...
def read_agent_version_from_source(commit: str, *, token: str = "", opener=urllib_request.urlopen) -> str:
    """Read the immutable candidate's declared package/module version."""
    url = f"https://raw.githubusercontent.com/{EDGE_AGENT_REPOSITORY}/{commit}/edge-agent/iot_cx_agent/__init__.py"
...
def parse_upgrade_request(body: bytes) -> UpgradeRequest:
    ...
    # git_ref is a compatibility alias for clients of the older form.
    entered_agent_commit = value(fields, "edge_agent_commit") or value(fields, "git_ref")
    override_requested = bool(entered_agent_commit and entered_agent_commit != manifest.agent_source_commit)
    if override_requested:
        try:
            resolved_override = resolve_commit(EDGE_AGENT_REPOSITORY, entered_agent_commit, token=os.environ.get("GITHUB_TOKEN", ""))
        except CommitResolutionError as exc:
            raise ValueError(str(exc)) from exc
        effective_agent_commit = resolved_override.full_sha
        expected_agent_version = read_agent_version_from_source(effective_agent_commit, token=os.environ.get("GITHUB_TOKEN", ""))
        agent_source = "Explicit operator override"
    else:
        effective_agent_commit = manifest.agent_source_commit
        expected_agent_version = manifest.edge_release
        agent_source = "Validated release manifest"
    request = UpgradeRequest(
        ...
        effective_agent_commit=effective_agent_commit,
...
def repo_commands(request: UpgradeRequest) -> list[tuple[str, str, bool]]:
    ...
    ref = shell_quote(request.effective_agent_commit)
    prerequisite_script = (
    ...
    return [
        ...
        ("repo release validation", repo_release_validation_command(request.remote_repo, request.effective_agent_commit), False),
    ]
...
def final_commands(request: UpgradeRequest, expected_bacnet_default_port: str = "47814", pre_restart_timestamp: str = "") -> list[tuple[str, str, bool]]:
    ...
    expected_agent = shell_quote(request.effective_agent_commit)
...
def validate_agent_runtime_output(output: str, request: UpgradeRequest, pre_restart_timestamp: str) -> None:
    values = {key: next((line.split("=", 1)[1] for line in output.splitlines() if line.startswith(key + "=")), "") for key in ("AGENT_RELEASE_COMMIT", "AGENT_PACKAGE_VERSION", "AGENT_MODULE_VERSION", "AGENT_SERVICE_STATE", "AGENT_SERVICE_START")}
    expected = {"AGENT_RELEASE_COMMIT": request.effective_agent_commit, "AGENT_PACKAGE_VERSION": request.expected_agent_version, "AGENT_MODULE_VERSION": request.expected_agent_version, "AGENT_SERVICE_STATE": "active"}
    mismatches = [f"{key}={values[key]!r}, expected {expected_value!r}" for key, expected_value in expected.items() if values[key] != expected_value]
    if not values["AGENT_SERVICE_START"] or values["AGENT_SERVICE_START"] in {"0", pre_restart_timestamp}:
        mismatches.append("AGENT_SERVICE_START is missing or unchanged from before restart")
    if mismatches:
...
class LegacyUpgradeRunner:
    ...
    def run_phase(self, index: int) -> None:
        ...
        try:
            if index == 0:
                ...
                output = self.run_commands(final_commands(self.request, JOBS[self.job_id].pre_upgrade_agent_default_port, JOBS[self.job_id].pre_restart_agent_timestamp), stop_on_failure=False)
    ...
    def write_preflight_summary(self, output: str) -> None:
        ...
        source_summary["Agent source commit"] = self.request.effective_agent_commit
            ...
            job.summary.update(
                {
                    ...
                    **source_summary,
                    ...
                    "AGENT_SOURCE_COMMIT": self.request.effective_agent_commit,
    ...
    def build_upload_zip(self) -> None:
        ...
        self.log.append(f"AGENT_SOURCE_COMMIT={self.request.effective_agent_commit}\n")
...
# DEV-UPDATER (6): the original polls the cloud for queued gateway-update jobs
# and runs them unattended. Jim's launcher suppresses that with -NoClaim by
# blanking the admin token in the process environment -- which cannot work here,
# because Steve's copied .env supplies the token from a file. This tool is
# manual by definition, and two updaters racing for the same queued job would be
# worse than either running alone, so claiming is off unless explicitly asked
# for. Nothing else about the worker changed.
CLAIM_ENV_VAR = "IOT_EDGE_DEV_UPDATER_CLAIM_CLOUD_JOBS"


def cloud_claiming_enabled() -> bool:
    return os.environ.get(CLAIM_ENV_VAR, "").strip().lower() in {"1", "true", "yes", "on"}


def run_server(port: int) -> BaseServer:
    # DEV-UPDATER (2): ask before binding, and name the other program's port if
    # that is the one that was asked for.
    ...
    if cloud_claiming_enabled():
        ...
        print("Cloud job claiming: ENABLED (explicitly requested)")
    else:
        with WORKER_STATUS_LOCK:
            WORKER_STATUS["state"] = "disabled (manual development use)"
        print("Cloud job claiming: disabled. This updater acts only when you press Update.")

    state = env_status()
    print(f"{identity.PRODUCT_NAME} {identity.APP_VERSION}: http://127.0.0.1:{port}")
    print(f"Edge UI   {DEFAULT_EDGE_UI_COMMIT_SHORT} ({DEFAULT_EDGE_UI_BRANCH})")
    print(f"Edge Agent {DEFAULT_EDGE_UPDATE_REF_SHORT} ({DEFAULT_EDGE_AGENT_BRANCH})")
    print(state.message())
    print(runtime.legacy_port_report())
    print("Press Ctrl+C to stop.")
    return server


def main() -> int:
    parser = argparse.ArgumentParser(description=f"{identity.PRODUCT_NAME} -- manual development use.")
