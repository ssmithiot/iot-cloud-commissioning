...
from iot_cx_agent.db import network_traffic_report, record_network_traffic, trend_transport_summary
...
def report(path: Path, now: datetime | None = None) -> dict[str, object]:
    result = network_traffic_report(path, now=now)
    result["trend_transports"] = trend_transport_summary(path)
    return result
