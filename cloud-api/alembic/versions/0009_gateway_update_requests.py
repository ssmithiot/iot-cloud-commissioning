...
def upgrade() -> None:
    ...
    if not inspector.has_table("gateway_update_requests"):
        op.create_table(
        ...
        inspector = sa.inspect(op.get_bind())
    indexes = {index["name"] for index in inspector.get_indexes("gateway_update_requests")}
    if "ix_gateway_update_requests_gateway_id" not in indexes:
        op.create_index("ix_gateway_update_requests_gateway_id", "gateway_update_requests", ["gateway_id"], unique=False)
    if "ix_gateway_update_requests_status" not in indexes:
