...
<script type="module">
  ...
  async function loadDeviceGraphic() {
    ...
    try {
      ...
      target.innerHTML = `<div class="bms-head"><div><span class="bms-kicker">Equipment graphic · Edge mirror · read only</span><h2>${escapeHtml(device.device_name || `Device ${device.device_instance}`)}</h2><span class="bms-sub">${escapeHtml(groups.get(device.group_id) || "Uncategorized")} · ${escapeHtml(template?.label || "Template not configured")}</span></div><div class="toolbar"><a class="button secondary" href="${pointsLink}">View All Points</a><a class="button secondary" href="/gateways/${encodeURIComponent(gatewayId)}/configure-tree">Configure Device</a></div></div><div class="rtu-graphic-grid">${roles.length ? roles.map((role) => `<article class="rtu-value-card"><span class="bms-label">${escapeHtml(roleLabel(role))}</span><div class="bms-value">${escapeHtml(pointValue(bound.get(role)))}</div><span class="bms-sub">${escapeHtml(bound.get(role)?.latest_read_at ? pointAge(bound.get(role).latest_read_at) : "Not mapped")}</span></article>`).join("") : `<article class="bms-tile wide"><span class="bms-label">Template not configured</span><p>Assign this mirrored device to a group and choose a graphic template before binding its points.</p></article>`}</div><p class="bms-sub">Values are last-known Edge mirror values; opening this page does not request BACnet reads.</p>`;
    } catch (error) {
  ...
  async function initConfigureTree() {
    ...
    if (manager) {
      manager.innerHTML = `<details class="mapping-manager"><summary>Point Mapping Templates (${mappingTemplates.length})</summary><p class="bms-sub">CSV is the reusable rule authority; applying a template never silently replaces manual bindings.</p><div class="mapping-template-list">${mappingTemplates.map((item) => `<article><strong>${escapeHtml(item.name)}</strong> · ${escapeHtml(item.graphic_template_key)}<ul>${item.rules.map((rule) => `<li>${escapeHtml(rule.logical_role)} ← ${escapeHtml(rule.match_field)} = ${escapeHtml(rule.match_value)}${rule.object_type ? ` (${escapeHtml(rule.object_type)})` : ""}</li>`).join("")}</ul><a class="button secondary" href="/api/ui/mapping-templates/${encodeURIComponent(item.id)}/export">Export CSV</a></article>`).join("") || "<p class=\"bms-sub\">No reusable mapping templates yet.</p>"}</div><label>Import Mapping Template CSV<textarea id="mapping-template-csv" rows="7" placeholder="template_name,graphic_template,logical_role,match_field,match_value,object_type,required"></textarea></label><button id="import-mapping-template" type="button">Import CSV</button></details>`;
      byId("import-mapping-template").onclick = async () => { await api("/api/ui/mapping-templates/import", {method:"POST", body:JSON.stringify(byId("mapping-template-csv").value)}); await initConfigureTree(); };
    }
