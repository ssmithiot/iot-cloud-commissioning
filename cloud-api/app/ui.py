...
def gateway_bms_shell_html(gateway_id: str, page: str, device_id: str | None = None) -> str:
  ...
  <main class="gateway-nav-content"><section class="gateway-bms-shell"><span class="eyebrow">Gateway BMS</span><h2>{title}</h2><p>{detail}</p>{'<div id="device-graphic" class="bms-shell"></div>' if page == 'device' else ''}{'<div id="configure-tree-body" class="configure-device-grid"></div><button id="save-configure-tree" type="button">Save configuration</button><div id="configure-tree-status" class="notice" aria-live="polite"></div>' if page == 'configure-tree' else ''}</section></main>
