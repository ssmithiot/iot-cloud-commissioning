...
<script type="module">
  ...
  async function initConfigureTree() {
    ...
    body.querySelectorAll(".save-device-configuration").forEach((button) => button.onclick = async () => {
      const card = button.closest("[data-config-device-id]"); const point_roles = Object.fromEntries([...card.querySelectorAll("select[data-point-id]")].map((select) => [select.dataset.pointId, select.value || null]));
      try { await api(`/api/ui/devices/${encodeURIComponent(card.dataset.configDeviceId)}/configuration`, {method:"PUT", body:JSON.stringify({group_name:card.querySelector("[data-device-group]").value, template_key:card.querySelector("[data-device-template]").value || null, mapping_template_id:card.querySelector("[data-mapping-template]").value || null, point_roles})}); card.querySelector(".mapping-result").textContent = `Saved successfully · Group: ${card.querySelector("[data-device-group]").value} · Graphic: ${card.querySelector("[data-device-template]").value || "none"} · Manual bindings saved: ${Object.values(point_roles).filter(Boolean).length}.`; await initConfigureTree(); } catch (error) { card.querySelector(".mapping-result").textContent = `Save failed: ${errorMessage(error)}`; }
    });
    body.querySelectorAll(".save-as-mapping-template").forEach((button) => button.onclick = async () => {
      ...
      try { const created = await api(`/api/ui/devices/${encodeURIComponent(card.dataset.configDeviceId)}/mapping-template`, {method:"POST", body:JSON.stringify({name})}); card.querySelector(".mapping-result").textContent = `Created mapping template: ${created.name} · Rules created: ${created.rules.length}.`; await initConfigureTree(); } catch (error) { card.querySelector(".mapping-result").textContent = `Template creation failed: ${errorMessage(error)}`; }
    });
  }
