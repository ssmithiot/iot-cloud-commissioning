...
class EdgeNode(Base):
    ...
    primary_mac: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class EdgeLocalTrendSample(Base):
    """Cloud mirror of a trend sample collected by a gateway's own trend store.

    The Edge owns local trend configuration: groups, points, intervals, enabled
    state and history all live on the gateway. These rows exist only so the
    cloud can display what the gateway already recorded, and are identified by
    the gateway-generated event_id so a retried upload cannot duplicate them.
    """
