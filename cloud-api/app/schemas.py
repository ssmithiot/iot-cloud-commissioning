...
class EdgeLocalTrendSampleIn(BaseModel):
    """One Edge-collected trend sample offered to the cloud mirror.

    event_id is generated by the gateway when the sample is recorded, and is
    what makes an upload retry idempotent.
    """
...
class EdgeLocalTrendSampleOut(EdgeLocalTrendSampleIn):
    gateway_id: str
    received_at: datetime | None = None
    model_config = ConfigDict(from_attributes=True)


class EdgeLocalTrendSampleAccepted(BaseModel):
    accepted: int
    duplicates: int
